...
perc_of_total_mth_view <- function(
  df,
  metric,
  group_col,
  filter_val,
...
###
  ...
  cur_yr_df <- df %>%
    ...
    group_by(yr_num, mth_num_in_yr, !!group_col) %>%
  ...
  prev_yr_df <- df %>%
    ...
    mutate(type = "actual") %>%
    # filter(between(wk_num_in_yr, prev_wk - (num_wks_to_show - 1), prev_wk)) %>%
    filter(UQ(group_col) == filter_val) %>% ### UQE not UQ #######
    ungroup()

  prev_yr_var_df <-
    right_join(
      cur_yr_df %>% select(mth_num_in_yr, perc_total, type),
      prev_yr_df %>% select(mth_num_in_yr, perc_total),
