...
if(!missing(metric_gaap)){

  metric_gaap <- enquo(metric_gaap)
  metric_gaap_name <- quo_name(metric_gaap)
...
if(!missing(metric_9p3)){
  cur_yr_df <- cur_yr_df %>%
    mutate(!!metric_cur_yr_name := if_else(is.na(!!metric_cur_yr), !!metric_9p3, !!metric_cur_yr)) %>%
    select(-!!metric_9p3)
}else if(!missing(metric_6p6)){
  cur_yr_df <- cur_yr_df %>%
    mutate(!!metric_cur_yr_name := if_else(is.na(!!metric_cur_yr), !!metric_6p6, !!metric_cur_yr)) %>%
    select(-!!metric_6p6)
...
###### join and handle gaap data ######

if(!missing(metric_gaap)){
  cur_yr_df <- cur_yr_df %>% left_join(df_gaap, by = "mth_num_in_yr") %>%
    mutate(!!metric_cur_yr_name := if_else(!is.na(!!metric_gaap), !!metric_gaap, !!metric_cur_yr)) %>%
    select(-!!metric_gaap)
}
